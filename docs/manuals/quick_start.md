...
1. `docs/operations/bringup_checklist.md` を確認します。
2. `docs/operations/maker_drive_initial_test_log.md` を記入します。
3. M1AがGPIO2、M1BがGPIO1に接続されていることを確認します。
4. 2ch試験を行う場合は、M2AがGPIO13、M2BがGPIO15に接続されていることを確認します。
5. 2ch操作UIは未実装であることを確認します。
6. 小型DCモータのみを接続します。
7. 1A連続を超えるモータを接続しないでください。
