...
- **Checklist**:
  - [ ] Enableコマンド非送信状態で、軸が拘束されていないことを安全な方法で確認する。
  - [ ] 必要に応じて、治具や安全なつまみ等で軸を低速に動かし、位置や速度のTelemetryが正しく受信できていることを確認する。
...
- **Date**: YYYY-MM-DD
- **Checklist**:
  - [ ] 初回負荷試験として、身体を巻き込まない既知の負荷（既知ばね・重り・治具など）をかけた際、指令値に対してドライバから妥当なTelemetry（電流増加等）が返ってくること。
...
- **絶対に手や指を反力・停止・負荷として使わないこと。低トルク設定であっても、手を負荷に使うことは認められない。**
